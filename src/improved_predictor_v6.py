"""
Improved predictor v6 - adds missing collapse lemmas and absorbing variable detection.
"""
import json
...
from cheatsheet_predictor import (
    ...
    constant_product_collapse, rename_vars, has_square, term_to_str,
    leftmost_var, rightmost_var
)


def check_collapse_lemma_v6(eq):
    """Extended collapse lemma check - includes simple projections."""
    lhs, rhs = eq

    # First check if it's bare
    if is_var(lhs) and not is_var(rhs):
        ...
        return check_collapse_lemma(eq)  # fallback for non-bare

    # Normalize
    norm = normalize_bare_law(eq)
    if norm is None:
        return check_collapse_lemma(eq)

    prod = norm[1]

    # Simple projection patterns (NEW):
    # x = y * x → RIGHT PROJECTION
    if prod == ('*', 'y', 'x'):
        return 'right_proj'

    # x = x * y → LEFT PROJECTION
    if prod == ('*', 'x', 'y'):
        return 'left_proj'

    # Additional projection patterns:
    # x = (y * x) * x → check: for all x,y: x = (y*x)*x.
    # Under right proj: (y*x)*x → x. LHS x → x. ✓
    # But need to verify this actually implies right proj.
    # Skip for now - not proven.

    # Now check the original 3 patterns
    # x = x*(y*(z*(x*y))) → LEFT PROJECTION
    lp1 = ('*', 'x', ('*', 'y', ('*', 'z', ('*', 'x', 'y'))))
    # x = x*((y*z)*(z*z)) → LEFT PROJECTION
    lp2 = ('*', 'x', ('*', ('*', 'y', 'z'), ('*', 'z', 'z')))
    if prod == lp1 or prod == lp2:
        return 'left_proj'

    # x = (((y*z)*x)*z)*x → RIGHT PROJECTION
    rp1 = ('*', ('*', ('*', ('*', 'y', 'z'), 'x'), 'z'), 'x')
    if prod == rp1:
        return 'right_proj'
...
def check_absorbing_source(eq):
    """Check if source equation has absorbing variables (appear on one side only).
    Returns 'absorbing_left' if a LHS-only variable exists (RHS doesn't depend on some LHS var),
    'absorbing_right' if a RHS-only variable exists,
    None otherwise."""
    lhs, rhs = eq
    lhs_vars = tree_vars_set(lhs)
    rhs_vars = tree_vars_set(rhs)

    # Variables in LHS but not RHS
    lhs_only = lhs_vars - rhs_vars
    # Variables in RHS but not LHS
    rhs_only = rhs_vars - lhs_vars

    if lhs_only and not is_var(lhs) and not is_var(rhs):
        # Non-bare equation with LHS-only variables
        # This means the result of the operation doesn't depend on those variables
        return 'absorbing_left', lhs_only
    if rhs_only and not is_var(lhs) and not is_var(rhs):
        return 'absorbing_right', rhs_only

    return None, None


def check_contradiction_motifs_v6(eq_a, eq_b):
...
def predict_v6(eq1_str, eq2_str):
    ...
    # STEP 0: Extended collapse lemmas
    if a_is_bare:
        collapse = check_collapse_lemma_v6(eq_a)
    ...
    # NEW: Check for absorbing variables in non-bare source
    abs_type, abs_vars = check_absorbing_source(eq_a)
    if abs_type == 'absorbing_left':
        # LHS has vars not in RHS → the operation result doesn't depend on those vars
        # This is extremely constraining. Almost everything is implied.
        # But we need to be careful: check if B satisfies the constraints
        # For now, use a high-confidence TRUE prediction for most cases
        fa_temp = compute_features(eq_a)
        fb_temp = compute_features(eq_b)
        if fa_temp['vars'] >= 3:
            return (True, 'absorbing_left', {})

    # STEP 0B: Contradiction motifs
    if a_is_bare:
        motif = check_contradiction_motifs_v6(eq_a, eq_b)
    ...
    # NEW: bare source with vA >= 5, past all guards → very likely trivializing
    if fa['bare'] and vA >= 5:
...
def evaluate_dataset(filepath):
    ...
    with open(filepath) as f:
        for line in f:
            row = json.loads(line)
            verdict, rule, _ = predict_v6(row['equation1'], row['equation2'])
            actual = row['answer']
            rule_stats[rule] += 1
            if verdict == actual:
                correct += 1
...
if __name__ == '__main__':
    ...
    for name, path in datasets:
        ...
        print(f"\n{name}: {result['correct']}/{result['total']} = {result['accuracy']:.4f} "
              f"(delta {delta:+.4f}) FP:{fp} FN:{fn}")
        for rule, count in sorted(result['rule_stats'].items(), key=lambda x: -x[1])[:15]:
            print(f"  {rule}: {count}")

    # Check specifically: absorbing_left accuracy
    print("\n\n=== ABSORBING LEFT ACCURACY ===")
    for name, path in datasets:
        abs_items = []
        with open(path) as f:
            for line in f:
                row = json.loads(line)
                verdict, rule, _ = predict_v6(row['equation1'], row['equation2'])
                if rule == 'absorbing_left':
                    abs_items.append({'actual': row['answer'], 'predicted': verdict, 'eq1': row['equation1'], 'eq2': row['equation2']})
        if abs_items:
            correct = sum(1 for i in abs_items if i['predicted'] == i['actual'])
            print(f"{name}: {correct}/{len(abs_items)} = {correct/len(abs_items):.3f}")
            for i in abs_items[:3]:
                if i['predicted'] != i['actual']:
                    print(f"  FP: {i['eq1']}  =>  {i['eq2']}")
