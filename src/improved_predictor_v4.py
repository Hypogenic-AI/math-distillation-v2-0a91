"""
Improved predictor v4 - surgical fixes based on motif analysis.
Key changes:
- C1: add topShape != m-m (removes all 5 FPs)
- C3: add rv >= 3 (removes 6 FPs)
- C6: add Rx=FALSE (removes all 20 FPs, keeps 20 TPs)
- C8: REMOVED (too unreliable)
- C9: REMOVED (too unreliable)
- C10: add RP_B=TRUE (removes 10 FPs)
- C13: add RP_B=TRUE (removes 21 FPs)
- C14: REMOVED (too unreliable)
- T4: add Lx=FALSE (removes all 9 FPs)
- T5, T7: REMOVED (too unreliable)
- T6: restrict to vA >= 3
- F2: REMOVED
- T1: moved before F1
"""
...
from cheatsheet_predictor import (
    ...
    constant_product_collapse, rename_vars, term_to_str
)


def check_contradiction_motifs_v4(eq_a, eq_b):
    ...
    # C1: FIXED - add topShape != m-m (removes all 5 FPs)
    if rv == 4 and not lx and not rx and ts != 'm-m':
        return 'C1'

    # C2: unchanged (95.8% accurate)
    if rv == 3 and rt == '113' and not lx and not rx:
        return 'C2'

    # C3: FIXED - add rv >= 3 (removes 6 of 7 FPs)
    ...
    # C6: FIXED - add Rx=FALSE (removes all 20 FPs, keeps 20 TPs)
    ...
    # C10: FIXED - add RP_B=TRUE (removes 10 FPs)
    if rv == 3 and rt == '122' and not lx and rx and xc == 2 and fb['RP']:
        return 'C10'

    # C13: FIXED - add RP_B=TRUE (removes 21 FPs)
...
def predict_v4(eq1_str, eq2_str):
    ...
    # STEP 0: Collapse lemmas (unchanged - 100%)
    if a_is_bare:
    ...
    if a_is_bare:
        motif = check_contradiction_motifs_v4(eq_a, eq_b)
    ...
    # STEP 2: Separators (unchanged - 100%)
    ...
    # T1: MOVED BEFORE F1 (was after F1)
...
def evaluate_dataset(filepath):
    ...
    with open(filepath) as f:
        for line in f:
            row = json.loads(line)
            verdict, rule, _ = predict_v4(row['equation1'], row['equation2'])
            actual = row['answer']
            rule_stats[rule] += 1
            if verdict == actual:
                correct += 1
...
if __name__ == '__main__':
    ...
    for name, path in datasets:
        ...
        print(f"\n{name}: {result['correct']}/{result['total']} = {result['accuracy']:.4f} (FP:{fp} FN:{fn})")
