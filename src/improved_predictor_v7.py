"""
Improved predictor v7 - remove absorbing_left, keep other v6 improvements.
Focus: targeted surgical fixes only.
"""
import json
...
from cheatsheet_predictor import (
    ...
    collapse_term, check_constant_product_lemma,
    constant_product_collapse, has_square
)


def check_collapse_lemma_v7(eq):
    """Extended collapse lemma check with simple projections."""
    lhs, rhs = eq
...
def check_contradiction_motifs_v7(eq_a, eq_b):
...
def predict_v7(eq1_str, eq2_str):
    ...
    if a_is_bare:
        collapse = check_collapse_lemma_v7(eq_a)
        if collapse is not None:
            b_lhs, b_rhs = eq_b
            cl = collapse_term(b_lhs, collapse)
            cr = collapse_term(b_rhs, collapse)
            if cl == cr:
                return (True, f'collapse_{collapse}', {})
            else:
                return (False, f'collapse_{collapse}_false', {})
    ...
    if a_is_bare:
        motif = check_contradiction_motifs_v7(eq_a, eq_b)
    ...
    # T1: after F1 (original position - avoids FPs from vB==vA cases)
    ...
    # NR: bare vA >= 4, topShape != m-m (past all guards, these almost always trivialize)
    if fa['bare'] and vA >= 4:
        if bsf and bsf['top_shape'] != 'm-m':
            return (True, 'NR_bare_v4', {})
        elif vA >= 5:  # vA>=5 always trivializes regardless of shape
